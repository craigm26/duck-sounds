...
let bp=null, bs=-9, br=null, evals=0;
const BUDGET = +(process.env.BUDGET || 320);
while (evals < BUDGET){
  let c = randP(), r = await attempt(c); evals++;
  for(let k=0;k<6 && evals<BUDGET;k++){
    const q = jitter(c,0.2), rq = await attempt(q); evals++;
    if (rq.score > r.score){ c=q; r=rq; k=0; }
  }
  if (r.score > bs){ bs=r.score; bp=c; br=r;
    console.log(`  wall: tilt ${r.tilt.toFixed(0)} deg, ends ${r.endUp?'upright':'down'} (${evals} evals)`); }
}
console.log(`WALLFLIP best tilt ${br.tilt.toFixed(0)} deg, ends ${br.endUp?'upright':'down'}`);
console.log(`WALLFLIP a full flip needs 180 deg AND landing upright`);
if (bp) fs.writeFileSync('wallflip-best.json', JSON.stringify({ r: br, p: bp }, null, 1));
