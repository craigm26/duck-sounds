...
mj.FS.writeFile('/scene.xml', fs.readFileSync('scene_rollers_visual.xml', 'utf8'));
...
const xml = fs.readFileSync('scene_rollers_visual.xml', 'utf8');
...
fs.writeFileSync('duck-visual-rollers.bin', Buffer.concat([
  len, header, Buffer.from(positions.buffer), Buffer.from(normals.buffer), Buffer.from(indices.buffer),
]));
console.log('SAVED duck-visual-rollers.bin', (fs.statSync('duck-visual-rollers.bin').size/1048576).toFixed(2), 'MB');
