// ac_success.mjs — measure_success.mjs pointed at the TRAINING-PARAMETER
// plant: ac_scene2.mjb + the ac_plant loop model (BAM m6 friction budget per
// substep, actuation delay Uniform{3..6} physics steps, joint_vel observation
// 1 control tick late). Recorded-endings baseline comes from the NEW corpus
// (ac_duck-intent-clips.json). Otherwise a faithful copy — including the
// HOME-as-neutral observation convention the old success numbers were
// measured with. Output: ac_intent-success.json.
//
...
async function capture(spec) {
  ...
  for (let t = -settle; t < ticks; t++) {
    ...
    const obs = buildObs([data.sensordata[GYRO], data.sensordata[GYRO + 1], data.sensordata[GYRO + 2]],
                         projectedGravity(quat()), jp, prevJv, lastAction, cmd);
    prevJv = jv;
...
const RECORDED = JSON.parse(fs.readFileSync('ac_duck-intent-clips.json', 'utf8')).clips;
...
fs.writeFileSync('ac_intent-success.json', JSON.stringify({
