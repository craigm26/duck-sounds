...
const BUDGET = +(process.env.BUDGET || 300);
for (let i = 0; i < BUDGET; ) {
  let cand = randP(), r = await score(cand); i++;
  for (let k = 0; k < 6 && i < BUDGET; k++){
    const q = jitter(cand, 0.2), rq = await score(q); i++;
    if (rq.score > r.score){ cand = q; r = rq; k = 0; }
  }
  if (r.score > bestS){ bestS = r.score; best = { p: cand, r };
    console.log(`  best so far: max tilt ${(Math.acos(Math.max(-1,Math.min(1,-r.over)))*57.3).toFixed(0)} deg from upright, ends ${r.endUp?'upright':'down'} (${i} evals)`); }
}
console.log(`BACKSEARCH best tilt ${(Math.acos(Math.max(-1,Math.min(1,-best.r.over)))*57.3).toFixed(0)} deg from upright; over the top needs 90+`);
console.log(`BACKSEARCH went past horizontal: ${best.r.over > 0 ? 'YES' : 'NO'}   ends upright: ${best.r.endUp}`);
fs.writeFileSync('backroll-best.json', JSON.stringify(best, null, 1));
