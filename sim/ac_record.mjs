// ac_record.mjs — record_intents.mjs pointed at the TRAINING-PARAMETER plant.
// Identical corpus, identical obs/ctrl contract, three deliberate differences:
//   1. scene: ac_scene2.mjb (solver 10/20, feet priority 1, training floor
//      friction, forcerange +-0.6405, stiff friction solref) instead of scene.mjb;
//   2. per-substep plant model from ac_plant.mjs: BAM m6 friction budget into
//      dof_frictionloss + actuation delay Uniform{3..6} physics steps on ctrl;
//   3. joint_vel observation delayed exactly 1 control tick (trained-in
//      Dynamixel firmware latency; ang_vel/gravity keep lag 0, inside their
//      trained Uniform{0,1}).
// Output: ac_duck-intent-clips.json. record_intents.mjs is untouched.
//
...
async function capture(spec) {
  ...
  let prevJv = new Array(14).fill(0);   // joint_vel reaches the actor 1 tick late
  const frames = [], roots = [];
  ...
  for (let t = -settle; t < ticks; t++) {
    ...
    // Delayed ctrl + per-substep BAM friction budget, then 4 x mj_step.
...
fs.writeFileSync('ac_duck-intent-clips.json', JSON.stringify(out));
console.log(`wrote ${Object.keys(out.clips).length} clips to ac_duck-intent-clips.json`);
